...
class Zombie:
    ...
    def runaway_to_boy(self, r=0.5):
        self.runaway_to_boy_move(play_mode.boy.x, play_mode.boy.y)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
        pass
    def runaway_to_boy_move(self, tx,ty):
    ...
    def count_ball_comparison(self):
    ...
    def build_behavior_tree(self):
        ...
        a3= Action('Set random locatrion',self.set_random_location)
        SEQ_wander = Sequence('Wander',a3,a2)

        c1 = Condition('소년이 근처에 있는가?',self.is_boy_nearby,7)

        c2 = Condition('소년보다 공의 개수가 적은가?',self.count_ball_comparison)
        a4 = Action('소년으로부터 도망', self.runaway_to_boy)
        SEQ_runaway = Sequence('도망', c2, a4)

        a5 = Action('소년으로부터 추적', self.move_to_boy)
        SEL_runaway_or_chase= Selector('도망 또는 추적', SEQ_runaway, a5)

        SEQ_chase_boy = Sequence('소년을 추적', c1,SEL_runaway_or_chase)
